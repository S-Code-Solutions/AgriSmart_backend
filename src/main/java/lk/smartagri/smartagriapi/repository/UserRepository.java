...
public interface UserRepository extends JpaRepository<User,Long> {
    ...
    @Modifying
    @Query(value = "UPDATE user SET address = ?1, email = ?2, id_photo = ?3, name = ?4, password = ?5, phone_no1 = ?6, phone_no2 = ?7, remarks = ?8, status = ?9, role_code = ?10 WHERE username = ?11",nativeQuery = true)
